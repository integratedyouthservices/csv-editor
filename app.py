...
def publish_dialog(n_rows: int, n_cells: int, n_added: int = 0, n_deleted: int = 0) -> None:
    ...
    if c2.button("Yes, publish", type="primary", width="stretch"):
        ...
        def record(row_id, column, old, new, change_type):
            return {
                "row_id": str(row_id), "column": column,
                "old_value": str(old), "new_value": str(new),
                "change_type": change_type, "timestamp": now,
                "user": user.username,
            }

        # One record per changed cell; a whole-row change logs every
        # column, blank on the side of the change that doesn't exist.
        audit_records = [
            record(row_id, column, df.at[row_id, column], value, "update")
            for (row_id, column), value in cells.items()
        ]
        inserts = []
        for row_id in added:
            values = row_values(df, row_id)
            ...
            audit_records += [
                record(row_id, column, "", value, "insert")
                for column, value in values.items()
            ]
        for row_id in deleted:
            audit_records += [
                record(row_id, column, value, "", "delete")
                for column, value in row_values(df, row_id).items()
            ]
        ...
        try:
            get_storage_provider().apply_changes(base, cells, inserts, deleted)
