...
- Adding and deleting rows is supported: the whole file is rewritten on publish either way (the shared `StorageProvider._frame_with_changes()` builds the new frame; `local_parquet`/`gcs_parquet` use the same helper). With `id_column` set, a row added in the editor must carry a value for that column or the publish is refused (a blank key would break the next load).
...
- Supports the app's CSV **Import** feature (`supports_import = True`): an imported file fully replaces the parquet object. The change log records the *diff* against what's published, matched on `dataset.identity_columns` — updates, inserts and deletes, not one `insert` per row.
- Supports **adding and deleting rows** in the editor: the object is rewritten wholesale on every publish anyway, so a row add or delete costs no more than a cell edit and lands in the same atomic PUT. `local_parquet` (the on-disk twin used for development) behaves identically.
...
Same pattern as auth: subclass `StorageProvider` (`providers/storage/base.py` documents the contract — `load()` returns a DataFrame indexed by a stable unique row id; `apply_edits()` persists `{(row_id, column): value}`; `apply_changes()` adds whole-row inserts/deletes — the base class refuses them rather than dropping them silently, and `_frame_with_changes()` does the work for any provider that rewrites its file wholesale; `write_audit()` and `replace_all()` are optional, as are the `audit_before_data_write`/`supports_import` class attributes that control ordering and gate the Import feature), register it in `providers/storage/__init__.py`, add a settings block, point `storage.provider` at it.
